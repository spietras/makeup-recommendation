...
from automakeup.feature.face import ClusteringIrisShapeExtractor, HoughCircleIrisShapeExtractor, \
    ThresholdingIrisShapeExtractor
...
class ThresholdingIrisShapeExtractorTestCase(unittest.TestCase):
    extractor = ThresholdingIrisShapeExtractor()

    def test_thresholding_iris_shape_extractor_returns_correct_shape(self):
        img, mask = get_eye("face.jpg", parser)
        iris_mask = self.extractor.extract(img, mask)
        self.assertEqual(iris_mask.shape[:2], img.shape[:2])

    def test_thresholding_iris_shape_extractor_returns_correct_type(self):
        img, mask = get_eye("face.jpg", parser)
        iris_mask = self.extractor.extract(img, mask)
        self.assertTrue(np.issubdtype(iris_mask.dtype, np.bool))

    def test_thresholding_iris_shape_extractor_can_find_the_iris(self):
        img, mask = get_eye("face.jpg", parser)
        iris_mask = self.extractor.extract(img, mask)
